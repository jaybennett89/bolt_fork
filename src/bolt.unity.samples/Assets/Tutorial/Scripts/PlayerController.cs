...
public class PlayerController : BoltEntityBehaviour<PlayerSerializer, IPlayerState> {
  bool left;
  bool right;
  bool forward;
  bool backward;
  ...
  TutorialPlayerMotor motor;

  void Awake () {
    motor = GetComponent<TutorialPlayerMotor>();
  }
  ...
  void Update () {
    if (boltEntity.boltIsControlling) {
      ...
      TutorialPlayerCamera.instance.rotateCameraBehindTarget = left || right || forward || backward;
    }
  }
  ...
  public override void SimulateController () {
    ...
    PlayerCommand playerCmd = BoltFactory.NewCommand<PlayerCommand>();
  ...
  public override void ExecuteCommand (BoltCommand cmd, bool resetState) {
    PlayerCommand playerCmd = (PlayerCommand) cmd;
