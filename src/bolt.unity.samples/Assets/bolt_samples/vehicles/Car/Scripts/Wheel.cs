...
public class Wheel : BoltEntityBehaviour {
  ...
  void FixedUpdate () {
    ...
    if (wheelModel != null) {
      ...
      if (boltEntity.boltIsOwner) {
        ...
        dummyAngle += (car.CurrentSpeed * 4);

        if (steerable) {
          wheelModel.localRotation = Quaternion.AngleAxis(car.CurrentSteerAngle, Vector3.up) * Quaternion.Euler(dummyAngle, 0, 0);
        } else {
          wheelModel.localRotation = Quaternion.Euler(dummyAngle, 0, 0);
        }
      }
