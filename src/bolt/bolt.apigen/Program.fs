...
let typeName (c:MemberReference) =
  c.Name.Replace("`1", "<T>")

let typeHref (c:MemberReference) =
  c.Name.Replace("`1", "_T")
...
type DocMethod = {
  ...
  member x.Name = (typeName x.Cecil.Value)
  member x.NameLink = (typeHref x.Cecil.Value.DeclaringType) + "_M_" + (typeHref x.Cecil.Value) + "()"
...
and DocProperty = {
  ...
  member x.Name = typeName x.Cecil.Value
  member x.NameLink = (typeHref x.Cecil.Value.DeclaringType) + "_P_" + (typeHref x.Cecil.Value)
...
//        | Some t -> [{DocTypeParam.Xml = t}]
  ...
  let types = 
    ...
    // find cecil type
    |> Seq.choose (fun x ->
      let cecil = dll.MainModule.Types |> Seq.tryFind (fun t -> t.FullName = x.Xml.Name.Substring(2))
...
//    )
  ...
  for t in types do
    p (sprintf "* [%s](#%s)" t.Name t.NameLink)

    // PROPERTIES

    for m in t.Properties |> Seq.distinctBy (fun p -> p.Name) do
      p (sprintf "  * [%s](#%s)" m.Name m.NameLink)
  ...
  for t in types do
    h3 (sprintf "<a name=\"%s\"></a> %s" t.NameLink t.Name)
    printSummary t.Xml
