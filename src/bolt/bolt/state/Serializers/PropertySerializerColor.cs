...
namespace Bolt {
  class PropertySerializerColor : PropertySerializerSimple {
    public override object GetDebugValue(State state) {
      var c = Blit.ReadColor(state.Frames.first.Data, SettingsOld.ByteOffset);
    ...
    protected override bool Pack(byte[] data, BoltConnection connection, UdpPacket stream) {
      stream.WriteColorRGBA(Blit.ReadColor(data, SettingsOld.ByteOffset));
      return true;
    }

    protected override void Read(byte[] data, BoltConnection connection, UdpPacket stream) {
      Blit.PackColor(data, SettingsOld.ByteOffset, stream.ReadColorRGBA());
    }
  }
}
