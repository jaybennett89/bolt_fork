  ...
  public override void Pack (BoltPacket packet) {
    ...
    for (int i = 0; i < _outgoingProxiesByNetworkId.Length; ++i) {
      ...
      if (proxy.flags & BoltEntityProxy.FLAG_DESTROY) {
        ...
        if (connection._flags & BoltConnection.FLAG_LOADING_MAP || BoltMapLoader.isLoading) { continue; }
        ...
        if (connection._flags & BoltConnection.FLAG_LOADING_MAP || BoltMapLoader.isLoading) {
          continue;
        }
