...
public class CarSelfRighting : BoltEntityBehaviour {

  // Automatically put the car the right way up, if it has come to rest upside-down.

  [SerializeField]
  private float waitTime = 3f;       	// time to wait before self righting
  [SerializeField]
  private float velocityThreshold = 1f;  // the velocity below which the car is considered stationary for self-righting
